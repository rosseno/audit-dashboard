...
# --- FITUR NOTIFIKASI / PERINGATAN OTOMATIS: REKOMENDASI MELEWATI BATAS RENCANA ---
# Berdasarkan bulan berjalan saat ini (Tahun 2026, bulan Agustus / atau dinamis dari sistem)
current_month_idx = datetime.now().month  # 1 sampai 12

# Logika Deteksi Overdue Berdasarkan Rencana Departemen & Status Tabel (SLS, EVAL, BD)
# Rekomendasi dianggap melewati batas jika statusnya belum Selesai (bukan SLS) 
# dan target bulan rencana departemen sudah terlewati (< current_month_idx).
if not df_base.empty:
    # Contoh simulasi pencocokan batas rencana berdasarkan dokumen rencana tiap bidang (Jan-Des 2026)
    # Misal batas toleransi rencana awal merujuk pada semester 1 / atau bulan lampau tahun berjalan.
    overdue_df = df_base[
    ...
    if overdue_count > 0:
        st.markdown(f"""
        <div style="background: rgba(239, 68, 68, 0.15); border: 1px solid #ef4444; padding: 15px 20px; border-radius: 8px; margin-bottom: 20px; display: flex; align-items: center; gap: 15px;">
            ...
            <div>
                ...
                <div style="color: #cbd5e1; font-size: 12px; margin-top: 3px;">Terdeteksi temuan dengan status Belum Selesai (BD/EVAL) yang telah melewati jadwal target rencana departemen tahun 2026. Mohon segera lakukan eskalasi.</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
